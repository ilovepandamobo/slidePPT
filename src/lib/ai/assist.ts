...
function staticAssistOutline(topic: string): string {
  return `# 封面：${topic}
# 目录
## 背景与现状
- 行业趋势
...
export async function assistOutline(
  ...
  const pageCount = duration
    ? Math.max(3, Math.min(20, Math.round(duration / 2)))
    : 8;

  if (isGrsaiChatConfigured()) {
    const content = await grsaiChatCompletion({
      messages: [
        {
          role: "system",
          content:
  ...
  return staticAssistOutline(topic);
}
      ...
      messages: [
        {
          role: "system",
          content:
            "根据用户对整套 PPT 的修改指令，输出一段英文 style prompt 追加描述（50词内）。",
        },
        {
          role: "user",
